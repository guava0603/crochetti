  ...
  <div class="wish-pool-view">
    <div class="page-content wish-pool-view__page">
      <div class="wish-pool-view__inner">
        ...
        <form class="wish-pool-form" @submit.prevent="handleSubmit" :style="stepFormStyle">
    ...
    <div
      ref="bottomToolbarEl"
      class="button-group-fixed button-group-fixed--page"
      role="region"
      :aria-label="$t('wishPool.submitBarAria')"
    >
      <div class="button-group button-group-fixed__inner">
        <button type="button" class="btn-primary" @click="handleSubmit" :disabled="loading || !canSubmit">
          {{ loading ? $t('wishPool.submitting') : $t('wishPool.submit') }}
        </button>
      </div>
    </div>
  </div>
...
import { useBottomToolbarPadding } from '@/composables/useBottomToolbarPadding'
...
async function handleSubmit() {
  ...
  if (!user) {
    await openError({
      ...
      message: t('wishPool.errors.loginRequired'),
