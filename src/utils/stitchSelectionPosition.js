...
import { getNodePerRepeatGenerate, getNodeTotalGenerate } from '@/utils/nodeGenerate'
...
// Craft-agnostic version of crochetPosition.computeGenerateDone.
export const computeGenerateDoneWithLookup = (selectionList, stitchList, counts, stitchLookup) => {
  ...
  for (let level = 0; level < selectionList.length; level += 1) {
    ...
    const index = selection && selection.start
    if (index === null || index === undefined) break

    for (let i = 0; i < index; i += 1) {
      total += getNodeTotalGenerate(currentList && currentList[i], stitchLookup)
    }

    const node = currentList && currentList[index]
    if (!node) break
    ...
    if (node.type === 'pattern') {
      const perRepeat = getNodePerRepeatGenerate(node, stitchLookup)
    ...
    if (node.type === 'stitch') {
      total += getNodePerRepeatGenerate(node, stitchLookup) * Math.max(0, selectedCount)
      return total
    }
...
// Craft-agnostic version of crochetPosition.endAtToSelectionList.
export function endAtToSelectionListWithLookup(row, endAt, stitchLookup) {
  ...
  function descend(nodes, count) {
    ...
    for (let i = 0; i < nodes.length; i += 1) {
      const node = nodes[i]
      const nodeGen = getNodeTotalGenerate(node, stitchLookup)

      if (remaining < nodeGen) {
        path.push({ start: i, end: i })

        if (node && node.type === 'pattern' && isArrayNonEmpty(node.pattern)) {
          const per = getNodePerRepeatGenerate(node, stitchLookup)
          const rawRemainder = per > 0 ? (remaining % per) : remaining
          const nextCount = rawRemainder === 0 && remaining > 0 ? per : rawRemainder

          // Keep the UI highlight stable: don't descend into the compact single-stitch pattern.
          if (!(node.pattern.length === 1 && node.pattern[0]?.type === 'stitch')) {
            descend(node.pattern, nextCount)
          }
        } else if (node && node.type === 'bundle' && isArrayNonEmpty(node.bundle)) {
          const per = getNodePerRepeatGenerate(node, stitchLookup)
          const rawRemainder = per > 0 ? (remaining % per) : remaining
          const nextCount = rawRemainder === 0 && remaining > 0 ? per : rawRemainder
      ...
      if (remaining === nodeGen) {
        ...
        if (node && node.type === 'pattern' && isArrayNonEmpty(node.pattern)) {
          const per = getNodePerRepeatGenerate(node, stitchLookup) || nodeGen
          if (!(node.pattern.length === 1 && node.pattern[0]?.type === 'stitch')) {
